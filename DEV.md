...
- Placing: `InkCanvas::textTapped(x, y)` fires on a one-finger tap on the glass (under 400 ms, under 24 px), on a mouse or touchpad click without drag in the pen or ruler tool (a mouse dot is never handwriting; the pen's dots are), or on a press/release without drag with the Text tool. Trap found 9 Sep: Qt reports a touchpad click with pointer type Finger; `inspectPointer` only treats a TouchScreen device as a finger, everything else is a mouse. A finger tap while the pen hovers within 1.5 s is ignored (palm rule), so with the pen in hand use the pen and the Text tool, or a click. QML: block under the tap → focus it; else if editing → leave; else `addTextAt`. Escape leaves. An empty block is removed on focus loss.
